/*
  Problem - Design an EventEmitter class. This interface is similar (but with some differences) to the one found in Node.js or the Event Target interface of the DOM.
...
class EventEmitter {
  ...
  subscribe(event, callback) {
    ...
    return {
      unsubscribe: () => {
        const index = this.#events[event]?.findIndex((eventInfo) => eventInfo.id === id);
        if (index >= 0) {
  ...
  emit(event, args = []) {
    ...
    return callbacks.map((cb) => cb.callback?.(...args));
  }
}